...
@export_range(0.1, 20.0, 0.1) var duration: float = 7.5:
    set(value):
        duration = value
        _queue_update()
...
func _initialize_nodes() -> void:
    ...
    if not positions_node:
        ...
        if not positions_node.is_inside_tree():
            ...
            if Engine.is_editor_hint() and get_tree().edited_scene_root:
                positions_node.owner = get_tree().edited_scene_root
                get_tree().edited_scene_root.set_edited(true)

    # Initialize start position marker
    if not start_position:
        ...
        if not start_position:
            ...
            if Engine.is_editor_hint() and get_tree().edited_scene_root:
                start_position.owner = get_tree().edited_scene_root
                get_tree().edited_scene_root.set_edited(true)
            start_position.position = start_pos

    # Initialize end position marker
    if not end_position:
        ...
        if not end_position:
            ...
            if Engine.is_editor_hint() and get_tree().edited_scene_root:
                end_position.owner = get_tree().edited_scene_root
                get_tree().edited_scene_root.set_edited(true)
...
func draw_path() -> void:
    ...
    path_line.default_color = Color(1, 1, 1, 0.3)

func create_default_curve() -> void:
    easing_curve = Curve.new()
    easing_curve.add_point(Vector2(0, 0), 0.0, 1.4)
    easing_curve.add_point(Vector2(1, 1), 1.4, 0.0)
